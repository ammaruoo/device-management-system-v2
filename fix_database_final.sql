...
-- إضافة عمود تاريخ الاستلام لجدول العمليات (إذا لم يكن موجوداً)
ALTER TABLE operations ADD COLUMN IF NOT EXISTS receipt_date DATE NULL;

-- إضافة عمود work_order_ref لجدول العمليات (إذا لم يكن موجوداً)
...
CREATE INDEX IF NOT EXISTS idx_repair_costs_operation ON repair_costs(operation_id);
